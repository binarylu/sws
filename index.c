...
generate_index(const char *path)
{
    ...
    if ((index_html = (char *)malloc(INDEX_BUFFER_SIZE)) != NULL) {
        ...
        if ((count = snprintf(pos, left, STR_HTML_DOCTYPE)) < 0) {
            perror("generate_index(): write error");
            free(index_html);
            return NULL;
        } else {
            pos += count;
        ...
        if((dirp = opendir(path)) == NULL) {
            perror("generate_index(): can't open path");
            free(index_html);
            return NULL;
        }
        while ((dp = readdir(dirp)) != NULL) {
            ...
            if (dp->d_name[0] != '.') {
                if ((count = snprintf(pos, left, "<tr><td><a href='%s/%s'>%s</a></td><td>%lu</td><td>%s</td></tr>",
                                path, dp->d_name, dp->d_name, sb.st_size, get_time_str(sb.st_mtime))) < 0) {
                    perror("generate_index(): write error");
                    free(index_html);
                    closedir(dirp);
                    return NULL;
